      ...
      <div className="min-h-screen">
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-lg shadow-indigo-500/5" : "bg-white/80 dark:bg-slate-900/80 backdrop-blur-md"} border-b border-gray-200 dark:border-slate-700/50`}>
          <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
            <a href="#" className="group/logo flex items-center gap-3 transition-all duration-300"><span className="block transition-all duration-300 group-hover/logo:scale-110 group-hover/logo:drop-shadow-[0_0_10px_rgba(99,102,241,0.4)]" style={{ transitionProperty: "transform, filter" }} onMouseEnter={(e) => { const el = e.currentTarget; el.style.transform = "scale(1.1) rotate(15deg)"; }} onMouseLeave={(e) => { const el = e.currentTarget; el.style.transform = "scale(1) rotate(0deg)"; }}><Logo size={28} /></span><span className="text-sm font-semibold text-gray-900 dark:text-white transition-all duration-300 group-hover/logo:tracking-wider">Mohammed Taqi Uddin</span></a>
